...
if Text and Text:match('(%d+)/play_wheel') then
  ...
  local media = {
    {
      "https://t.me/cr_banko_update/3","مبروك ربحت 10000000 دولار 💵","10000000"
    },
    {
      "https://t.me/cr_banko_update/3","مبروك ربحت 5000000 دولار 💵","5000000"
    },
    {
      "https://t.me/cr_banko_update/3","مبروك ربحت 1000000 دولار 💵","1000000"
    },
    {
      "https://t.me/cr_banko_update/3","مبروك ربحت 100000 دولار 💵","100000"
    },
    {
...
return bot.answerCallbackQuery(data.id, "شنو رئيك نزوجك مكانهم", true)
end
...
return bot.editMessageText(chat_id,msg_id,"خلص الطلب وين جنتي من طلب ايدج", 'md', false)
end
end
end
...
return bot.editMessageText(chat_id,msg_id,"خليج ييمعدله هيج لحد م يطلكج", 'md', false)
end
end
---
...
if tonumber(IdUser) == tonumber(UserId) then
Redis:set(Fast..'takforall'..ChatId,'open')
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = '- رجوع', data =UserId..'/'.. 'listallAddorrem'},},}}
...
if tonumber(IdUser) == tonumber(UserId) then
Redis:set(Fast..'all:members'..ChatId,'open')
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = '- رجوع', data =UserId..'/'.. 'BackTac'},},}}
...
if tonumber(IdUser) == tonumber(UserId) then
Redis:set(Fast..'lockalllll'..ChatId,'on')
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = '- رجوع', data =UserId..'/'.. 'BackTac'},},}}
...
if tonumber(IdUser) == tonumber(UserId) then
Redis:set(Fast..'takforall'..ChatId,'close')
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = '- رجوع', data =UserId..'/'.. 'listallAddorrem'},},}}
...
if tonumber(IdUser) == tonumber(UserId) then
Redis:set(Fast..'all:members'..ChatId,'close')
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = '- رجوع', data =UserId..'/'.. 'BackTac'},},}}
...
if tonumber(IdUser) == tonumber(UserId) then
Redis:set(Fast..'lockalllll'..ChatId,'off')
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = '- رجوع', data =UserId..'/'.. 'BackTac'},},}}
...
Redis:incrby(Fast.."Mero:Num:Add:Games"..ChatId..name1,3)  
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
{
...
Redis:srem(Fast..'loce:list'..ChatId,name1)
Redis:incrby(Fast.."Mero:Num:Add:Games"..ChatId..name1,5)  
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
{
...
if #list == 1 then
Redis:srem(Fast..'rolet:list'..ChatId,name)
Redis:incrby(Fast.."Mero:Num:Add:Games"..ChatId..IdUser,10)  
...
Redis:sadd(Fast.."Mero:SilentGroup:Group"..ChatId,GetUserReply[1]) 
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {
{
...
local UserInfo = bot.getUser(GetUserReply[1])
local Teext = ' ['..UserInfo.first_name..'](tg://user?id='..GetUserReply[1]..')'
Redis:sadd(Fast.."Mero:BanGroup:Group"..ChatId,GetUserReply[1]) 
...
Redis:srem(Fast.."Mero:Distinguished:Group"..ChatId,GetUserReply[1]) 
...
Redis:srem(Fast.."Mero:BanGroup:Group"..ChatId,GetUserReply[1]) 
Redis:srem(Fast.."Mero:SilentGroup:Group"..ChatId,GetUserReply[1]) 
...
if Redis:sismember(Fast.."Mero:Num:okiktm"..ChatId, IdUser) then
return bot.answerCallbackQuery(data.id, "⇜ قمت بالتصويت مسبقاً", true)
end
Redis:sadd(Fast.."Mero:Num:okiktm"..ChatId, IdUser)  
local list = Redis:smembers(Fast.."Mero:Num:okiktm"..ChatId)
if #list >= tonumber(3) then
Redis:del(Fast.."Mero:Num:okiktm"..ChatId)
...
Redis:sadd(Fast.."Mero:SilentGroup:Group"..ChatId,GetUserReply) 
end
...
local UserInfo = bot.getUser(GetUserReply)
local Teext = ' ['..UserInfo.first_name..'](tg://user?id='..GetUserReply..')'
if Redis:sismember(Fast.."Mero:Num:noiktm"..ChatId, IdUser) then
return bot.answerCallbackQuery(data.id, "⇜ قمت بالتصويت مسبقاً", true)
end
Redis:sadd(Fast.."Mero:Num:noiktm"..ChatId, IdUser)  
local list = Redis:smembers(Fast.."Mero:Num:noiktm"..ChatId)
if #list >= tonumber(3) then
Redis:del(Fast.."Mero:Num:noiktm"..ChatId)
Redis:srem(Fast.."Mero:SilentGroup:Group"..ChatId,GetUserReply) 
...
keyboard = {} 
keyboard.inline_keyboard = {
{{text = '𝕊𝕆𝕌ℝℂ𝔼 ℂ𝔸ℝ𝕆𝕃𝕌𝕊',url='http://t.me/'..GG_EER..''}},
}
...
if name == "✂" then
Redis:incrby(Fast.."Mero:Num:Add:Games"..ChatId..IdUser, 1)  
...
data = {
{
{text = '• 𝑆𝑂𝑈𝑅𝐶𝐸 𝑉𝐴𝐿𝑂𝑅𝐴𝑁𝑇 •', url = 't.me/n_n_c_2'}, 
},
}
}
...
data = {
{
{text = '{ 𝟙 }', data = IdUser..'/help1'}, {text = '{𝟚}', data = IdUser..'/help2'}, 
},
{
{text = '{𝟛}', data = IdUser..'/help3'}, {text = '{ 𝟜 }', data = IdUser..'/help4'}, 
},
{
{text = '{ 𝟝}', data = IdUser..'/help5'}, {text = '{ 𝟞 }', data = IdUser..'/helpp6'}, 
},
{
...
bot.answerCallbackQuery(data.id, "• تم معادرة البوت من الكروب", true)
...
edit(ChatId,Msg_id,"• تم مسح ادامن الكروب", "md",true, false, reply_markup)
end
